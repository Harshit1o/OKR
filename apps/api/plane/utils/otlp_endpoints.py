...
(e.g. https://telemetry.scalex.example or https://telemetry.scalex.example behind
...
_DEFAULT_OTLP_ENDPOINT = "https://telemetry.scalex.example"


def grpc_endpoint_from_url(url: str) -> str:
    """
    Derive gRPC host:port from OTLP_ENDPOINT URL.
    - https://telemetry.scalex.example -> telemetry.scalex.example:443 (nginx ingress)
    - https://telemetry.scalex.example -> telemetry.scalex.example:443 (dev)
    - telemetry.scalex.example:4317 -> telemetry.scalex.example:4317 (scheme-less with port)
    - telemetry.scalex.example -> telemetry.scalex.example:4317 (scheme-less, default gRPC port)
    ...
    if "://" not in url:
        url = "//" + url
    parsed = urlparse(url)
    host = parsed.hostname or "telemetry.scalex.example"
