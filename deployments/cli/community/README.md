...
You have successfully self hosted `ScaleX` instance. Access the application by going to IP or domain you have configured it (e.g `https://plane.example.com:8080` or `http://[IP-ADDRESS]:8080`)
...
### Upgrading ScaleX Version 

It is always advised to keep ScaleX up to date with the latest release.
...
1. Make sure that ScaleX-CE is installed, started, and then stopped. This ensures that the Docker volumes are created.
...
1. Start the ScaleX instance using `./setup.sh start`.

---

### Restore for Commercial Air-Gapped (Docker Compose)

When you want to restore the previously backed-up data on ScaleX Commercial Air-Gapped version, follow the instructions below.
...
1. Make sure that ScaleX Commercial (Airgapped) is extracted and ready to get started. In case it is running, you would need to stop that.
...
1. After restoration, you are ready to start ScaleX Commercial (Airgapped) will all your previously saved data. 
