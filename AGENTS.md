...
The Django/pytest suite for `apps/api` runs in an isolated stack defined by `docker-compose-test.yml` at the repo root. (Internal Python modules retain the `plane.*` namespace for compatibility; this is intentional and unrelated to the ScaleX brand.)
